...
struct cdb_options { /* a file abstraction layer, could point to memory, flash, or disk */
	...
	void *(*open)(cdb_t *cdb, const char *name, int mode); /* open up a resource, which may or may not be a file, for reading (mode = CDB_RO_MODE) or read/write (mode = CDB_RW_MODE) */
	int (*close)(cdb_t *cdb, void *file); /* close a resource opened up with "open" */
...
CDB_API void *cdb_reallocate(cdb_t *cdb, void *pointer, const size_t length);
